...
sizes = [2 ** i for i in range(17)]  # 2**16 is the limit
...
with PdfPages(results_folder + report_filename) as pdf:
    ...
    fig.suptitle(
        'CPU Cycles and Runtime Comparision of nFFT and vFFT', fontsize=14)

    # Speedup Plot (Time)
    axs[1, 1].plot(sizes, speedup_time, label='Speedup (Time)',
                   marker='o', color='blue')
    ...
    # Speedup Plot (Cycles)
    axs[0, 1].plot(sizes, speedup_cycles,
                   label='Speedup (Cycles)', marker='x', color='red')
    ...
    axs[1, 0].bar(x - width/2, nFFT_times, width,
                  label='nFFT Time', color='blue')
    axs[1, 0].bar(x + width/2, vFFT_times, width,
                  label='vFFT Time', color='orange')
    ...
    # Bar Plot (Cycles)
    axs[0, 0].bar(x - width/2, nFFT_cycles, width,
                  label='nFFT Cycles', color='blue')
    axs[0, 0].bar(x + width/2, vFFT_cycles, width,
                  label='vFFT Cycles', color='orange')
    ...
    # vFFT on left
    axs[0, 0].plot(df['size'], avg_vFFT_error,
                   label='vFFT Avg Error', marker='x')
    axs[0, 0].fill_between(df['size'], min_vFFT_error,
                           max_vFFT_error, alpha=0.3, label='vFFT Min-Max Error')
    ...
    # nFFT on right
    axs[0, 1].plot(df['size'], avg_nFFT_error,
                   label='nFFT Avg Error', marker='x')
    axs[0, 1].fill_between(df['size'], min_nFFT_error,
                           max_nFFT_error, alpha=0.3, label='nFFT Min-Max Error')
    ...
    axs[1, 0].plot(df['size'], avg_vIFFT_error,
                   label='vIFFT Avg Error', marker='o')
    axs[1, 0].fill_between(df['size'], min_vIFFT_error,
                           max_vIFFT_error, alpha=0.3, label='vIFFT Min-Max Error')
    ...
    axs[1, 1].plot(df['size'], avg_nIFFT_error,
                   label='nIFFT Avg Error', marker='o')
    axs[1, 1].fill_between(df['size'], min_nIFFT_error,
                           max_nIFFT_error, alpha=0.3, label='nIFFT Min-Max Error')
    ...
    plt.fill_between(df['size'], min_vFFT_error, max_vFFT_error,
                     alpha=0.3, label='vFFT Min-Max Error')
    ...
    plt.fill_between(df['size'], min_vIFFT_error,
                     max_vIFFT_error, alpha=0.3, label='vIFFT Min-Max Error')
    ...
    error_stats_vFFT = np.array(
        [avg_vFFT_error, min_vFFT_error, max_vFFT_error, std_vFFT_error])
    error_stats_df_vFFT = pd.DataFrame(error_stats_vFFT.T, columns=[
                                       'Avg', 'Min', 'Max', 'Std'], index=df['size'])

    # Create the error statistics DataFrame for nFFT
    error_stats_nFFT = np.array(
        [avg_nFFT_error, min_nFFT_error, max_nFFT_error, std_nFFT_error])
    error_stats_df_nFFT = pd.DataFrame(error_stats_nFFT.T, columns=[
                                       'Avg', 'Min', 'Max', 'Std'], index=df['size'])
    ...
    sns.heatmap(error_stats_df_vFFT, annot=True, fmt='.2f',
                cmap='coolwarm', cbar_kws={'label': 'Error Value'}, ax=axes[0])
    axes[0].set_title("vFFT")
    axes[0].set_ylabel('Input Size')

    # Plot the heatmap for nFFT
    sns.heatmap(error_stats_df_nFFT, annot=True, fmt='.2f',
                cmap='coolwarm', cbar_kws={'label': 'Error Value'}, ax=axes[1])
    axes[1].set_title("nFFT")
    axes[1].set_ylabel('Input Size')

    # Adjust layout
    fig.suptitle(
        'Heatmap of Errors (Avg, Min, Max, Std) Across Sizes', fontsize=14)
    ...
    plt.figure(figsize=fig_size)
    plt.loglog(df['size'], avg_vFFT_error,
               label='vFFT Average Error', marker='x')
    plt.loglog(df['size'], avg_vIFFT_error,
               label='vIFFT Average Error', marker='x')
    plt.loglog(df['size'], avg_nFFT_error,
               label='nFFT Average Error', marker='o')
    plt.loglog(df['size'], avg_nIFFT_error,
               label='nIFFT Average Error', marker='o')
