def process_file(file_name, start_index, end_index):
    real = []
    imag = []
...
def read_file_reverse_and_process(file_path):
...
if __name__ == "__main__":
    ...
    # now real and imag vals have some long words
    # waht i need to do is break each string in realVal and imagVal into 8 size word in revrse order
    real = []
    img = []
    
    for i in range(len(realVal)):
        realVector = realVal[i]
        imagVector = imagVal[i]
        ...
        real.extend(realVector)
        img.extend(imagVector)
        
    realVal = hex_to_float(real)
    imagVal = hex_to_float(img)
